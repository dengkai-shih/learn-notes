$org_root_x = "E:\71-presentation\99-gunini\2022\10\25"
...
#echo $org_files
$org_root = $org_root_x + "\01-photo"
if (Test-Path -Path $org_root) {
    ...
    foreach ($item in $org_files) {
        ...
        if (Test-Path -Path $folderPath) {
            try {
                ...
                Write-Host "move file failed : "$item.FullName" to "$folderPath
            }
        }
    }
} else {
    Write-Host "path is not existed : "$org_root
}
$org_root = $org_root_x + "\02-video"
if (Test-Path -Path $org_root) {
    $org_files = (Get-ChildItem $org_root | Select-Object FullName)
    foreach ($item in $org_files) {
        $date = (Get-Item $item.FullName).LastWriteTime | Get-Date -Format "yyyy-MM-dd"
